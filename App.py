...
class App:
    ...
    def __init__(self):
        ...
        self.layout = [
            ...
            [ sg.Text("Rementente") ,sg.Input(key="nodeIn",size=(10,10)), 
              ...
              sg.Text("KM total: ", key="weight",size=(15, 1))
            ]
        ...
        self.window = sg.Window('Consultando a Melhor Rota', self.layout)
    ...
    def prepare_circles(self):
        circleList = {}
        circleList['A'] =  self.graph.DrawImage(filename="local.png", location=(50,365))   # 0
        circleList['B'] =  self.graph.DrawImage(filename="local.png", location=(135,225))  # 1
        circleList['C'] =  self.graph.DrawImage(filename="local.png", location=(200,450))  # 2
        circleList['D'] =  self.graph.DrawImage(filename="local.png", location=(275,145))  # 3
        circleList['E'] =  self.graph.DrawImage(filename="local.png", location=(375,355))  # 4 
        circleList['F'] =  self.graph.DrawImage(filename="local.png", location=(525,345))  # 5 
        circleList['G'] =  self.graph.DrawImage(filename="local.png", location=(475,165))  # 6 
        return circleList
    
    def prepare_labels(self):
        self.graph.DrawText(text= 'A', location=(70,345),   color="red", font=16, angle=0, text_location="center")
        self.graph.DrawText(text= 'B', location=(180,205),  color="red", font=16, angle=0, text_location="center")
        self.graph.DrawText(text= 'C', location=(220,430),  color="red", font=16, angle=0, text_location="center")
        self.graph.DrawText(text= 'D', location=(315, 130), color="red", font=16, angle=0, text_location="center")
        self.graph.DrawText(text= 'E', location=(385, 325), color="red", font=16, angle=0, text_location="center")
        self.graph.DrawText(text= 'F', location=(550, 325), color="red", font=16, angle=0, text_location="center")
        self.graph.DrawText(text= 'G', location=(510, 145), color="red", font=16, angle=0, text_location="center")
        ...
        while True:
            ...
            if event == sg.WIN_CLOSED or event == 'Exit':
                ...
                if self.start not in self.babel.keys() or self.end not in self.babel.keys():
                    sg.popup_ok('Local(is) informado(s) não existe(m)', title="Erro",auto_close=False) 
                    continue
                elif self.start == self.end:
                    self.window['weight'].update("KM total: 0")
                    continue
                result = self.theGraph.ucs(self.babel[self.start],self.babel[self.end])
                if(len(result) > 0):
                    path   = result[0]
                    weight = result[1]
                    self.window['weight'].update("KM total: %d"%(weight))
